  ...
  public:
    ...
    explicit user_data_header(std::string_view buf)
    {
        ...
        while (buf.size() >= header_length)
        {
            ...
            if ((tag == concatenated_sm_8bit_ref && val_buf.size() != 3) || (tag == concatenated_sm_16bit_ref && val_buf.size() != 4))
                throw std::runtime_error{ "user_data_header multi_part_data length is invalid" };
    ...
    void set_multi_part_data(const multi_part_data& mpd)
    {
        ...
        else
        {
            headers_.emplace_back(
                ...
                std::string{ { static_cast<char>(mpd.concat_sm_ref_num_), static_cast<char>(mpd.number_of_parts_), static_cast<char>(mpd.sequence_number_) } });
        }
    }
    ...
    {
        {
            const auto it = std::find_if(headers_.begin(), headers_.end(), [](const auto& h) { return h.first == concatenated_sm_8bit_ref; });
            if (it != headers_.end())
                return { static_cast<uint16_t>(it->second[0]), static_cast<uint8_t>(it->second[1]), static_cast<uint8_t>(it->second[2]) };
        }

        {
            const auto it = std::find_if(headers_.begin(), headers_.end(), [](const auto& h) { return h.first == concatenated_sm_16bit_ref; });
            if (it != headers_.end())
                return { static_cast<uint16_t>(static_cast<uint16_t>(it->second[0]) << 8 | static_cast<uint16_t>(it->second[1])),
...
inline std::pair<user_data_header, std::string> unpack_short_message(esm_class esm_class, data_coding data_coding, const std::string& short_message)
{
    if (extract_unicode(data_coding) == data_coding_unicode::ascii_8_bit && short_message.length() > 160)
        throw std::runtime_error{ "unpacking short_message failed, short_message length is larger than 160" };

    if (extract_unicode(data_coding) != data_coding_unicode::ascii_8_bit && short_message.length() > 140)
        throw std::runtime_error{ "unpacking short_message failed, short_message length is larger than 140" };

    if (esm_class.gsm_network_features == gsm_network_features::udhi || esm_class.gsm_network_features == gsm_network_features::both)
    {
        const auto udh_length = static_cast<uint8_t>(short_message[0]);

        if (udh_length >= short_message.length())
            throw std::runtime_error{ "unpacking short_message failed, UDH lenght is larger than short_message" };
...
inline std::string pack_short_message(const user_data_header& user_data_header, std::string_view body, data_coding data_coding)
{
    ...
    if (extract_unicode(data_coding) == data_coding_unicode::ascii_8_bit && short_message.length() > 160)
        throw std::runtime_error{ "packing short_message failed, short_message length is larger than 160" };

    if (extract_unicode(data_coding) != data_coding_unicode::ascii_8_bit && short_message.length() > 140)
        throw std::runtime_error{ "packing short_message failed, short_message length is larger than 140" };
