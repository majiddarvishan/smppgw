...
class oparam
{
  ...
  public:
    ...
    {
        auto deserialize_u16 = [](std::span<const uint8_t, 2> data) -> uint16_t { return data[0] << 8 | data[1]; };
    ...
    void serialize(std::vector<uint8_t>* vec) const
    {
        ...
        for (const auto& param : oparam_)
        {
            auto tag = param.first;
            const auto& buf = param.second;
